...
export const WorkforceCensusModal: React.FC<WorkforceCensusModalProps> = ({
  ...
  const [breakdown, setBreakdown] = useState<Partial<Record<ProfessionalCategory, number>>>(() => {
    // If unit has cnesProfessionals loaded, compute live active count
    const unitProfs = cnesProfessionals.filter(p => p.unitId === unit.id && p.status === 'Ativo');
    if (unitProfs.length > 0) {
      const calculated: Partial<Record<ProfessionalCategory, number>> = {};
      unitProfs.forEach(p => {
        calculated[p.professionalCategory] = (calculated[p.professionalCategory] || 0) + 1;
      });
      return calculated;
    }
    return unit.activeStaffBreakdown || {
  ...
  const [notes, setNotes] = useState(
    `Quadro atualizado com base na base oficial do CNES (${unit.cnes || 'DATASUS'}) e escala do mês.`
  );
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col justify-between gap-1.5">
              <span className="text-xs font-semibold text-slate-300 mb-0.5">Sincronização & Ações</span>
              <div className="grid grid-cols-2 gap-1.5">
                <button
                  type="button"
                  onClick={() => {
                    // Populate from cnesProfessionals if available, or unit activeStaffBreakdown
                    const unitProfs = cnesProfessionals.filter(p => p.unitId === unit.id && p.status === 'Ativo');
                    if (unitProfs.length > 0) {
                      const calculated: Partial<Record<ProfessionalCategory, number>> = {};
                      unitProfs.forEach(p => {
                      ...
                      setSyncToast(`Sincronizado! ${unitProfs.length} profissionais ativos importados do CNES (${unit.cnes || '0002135'}).`);
                    } else {
                      const cnesCount = unit.activeStaffBreakdown || {};
                      setBreakdown({ ...cnesCount });
                      setSyncToast(`Sincronizado com base CNES (${unit.cnes || 'DATASUS'}).`);
                    }
                    setNotes(`Dados sincronizados com a base oficial do CNES (${unit.cnes || 'DATASUS'}) em ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}.`);
                    setTimeout(() => setSyncToast(null), 4000);
                  }}
                  className="py-2 px-2 bg-blue-900/80 hover:bg-blue-800 text-blue-200 border border-blue-600/70 rounded-lg text-[11px] font-bold flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-sm active:scale-95"
                  title="Puxar quantitativos da base CNES"
                >
                  <RefreshCw className="w-3.5 h-3.5 text-blue-300 animate-none hover:rotate-180 transition-transform duration-500" />
                  <span>Puxar CNES</span>
                </button>
                <button
                  type="button"
                  onClick={() => setImportMode(!importMode)}
                  className="py-2 px-2 bg-slate-800 hover:bg-slate-750 text-slate-200 border border-slate-700 rounded-lg text-[11px] font-medium flex items-center justify-center gap-1 transition-colors cursor-pointer"
                >
                  <UploadCloud className="w-3 h-3 text-teal-400" />
                  <span>{importMode ? 'Formulário' : 'Colar Dados'}</span>
                </button>
              </div>
            </div>
          </div>
          ...
          {syncToast && (
            <div className="bg-emerald-950/80 border border-emerald-700 text-emerald-300 text-xs px-3.5 py-2.5 rounded-lg flex items-center gap-2 animate-fadeIn">
