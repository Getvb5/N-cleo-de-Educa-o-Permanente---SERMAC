...
interface AuthScreenProps {
  ...
  onOpenPublicIndicatorsForm?: (unitId?: string) => void;
}

export const AuthScreen: React.FC<AuthScreenProps> = ({ units, onLoginSuccess, onOpenPublicIndicatorsForm }) => {
  ...
  return (
    <RecifeBackground>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8 flex flex-col justify-center">
        ...
        {/* PUBLIC ACCESS BANNER: COLETA DE INDICADORES (NO LOGIN REQUIRED) */}
        {onOpenPublicIndicatorsForm && (
          <div className="max-w-6xl mx-auto w-full mb-6 bg-linear-to-r from-[#0C326F]/95 via-[#103E8A]/95 to-[#1351B4]/95 backdrop-blur-md rounded-2xl border border-white/30 p-4 sm:p-5 shadow-xl text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider bg-emerald-500/30 text-emerald-200 border border-emerald-400/40 flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3 text-emerald-300" />
                  Acesso Público Liberado
                </span>
                <span className="text-xs text-blue-200 font-semibold">
                  Sem necessidade de senha ou cadastro
                </span>
              </div>
              <h3 className="text-base sm:text-lg font-bold text-white tracking-tight flex items-center gap-2">
                Formulário de Coleta de Indicadores e Censo (NEPS)
              </h3>
              <p className="text-xs text-blue-100 max-w-2xl leading-relaxed">
                Qualquer profissional, responsável técnico ou comissão com o link pode preencher e transmitir a apuração de indicadores e o censo da sua unidade de saúde diretamente, sem precisar efetuar login no sistema.
              </p>
            </div>

            <button
              type="button"
              onClick={() => onOpenPublicIndicatorsForm()}
              className="px-4 py-2.5 rounded-xl bg-white text-[#0C326F] hover:bg-blue-50 font-bold text-xs flex items-center gap-2 shadow-md transition shrink-0 cursor-pointer hover:scale-[1.02]"
            >
              <FileSpreadsheet className="w-4 h-4 text-[#1351B4]" />
              <span>Acessar Formulário Público</span>
              <ArrowRight className="w-4 h-4 text-[#1351B4]" />
            </button>
          </div>
        )}
